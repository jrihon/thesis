...
The DNA strands are then transcribed (_transcription_) to single stranded (ss) RNA.
...
Important to clarify is that not the entire genome is fit for translation, but is divided into _introns_ and _exons_. These exons are the DNA sequences that are transcribed to become messenger RNA (mRNA). When transcribed, this mRNA is escorted outside of the nucleus and accepted by a collection of proteins to form the ribosome. This complex reads the NA sequence by the codon - a triplet of nucleobases - and forms a growing strand of amino acids, whose composition is determined by the codons read from mRNA sequence itself (_translation_). 
...
Another example is that of _ribozymes_, capable of catalytic activity. These include folded RNA strands that can splice ssRNA, and even an _in vitro_ RNA polymerase ribozyme was designed that is capable of extending the complementary strand of an RNA template #mcite(("McRae2024ribozyme"), biblio).
//
//
//
//
...
Most remarkable alterations are brought forth from Kool's and Benner's group respectively. One is that of (non-)canonical hydrogen bonding patterns. This is effectuated by altering the purine:pyrimidine dyad.
...
//    Highlights in turquoise of synthetic modifications.
    Synthetic modifications are highlighted in turquoise.
...
As we will come to learn, the backbone characterises how the helix bends and twists and the nucleobase modifications will determine the interactions with their target. 
...
//
==== Antiviral therapy
The application of nucleoside analogs as a therapeutic intervention to a viral infection hampers the replicative cycle of the virus or completely neutralises it, the latter being extremely difficult to achieve with drugs alone.
...
The antisense oligonucleotides (ASOs) are a particular class of drugs that are administered in order to emulate the effects of the endogenous RNAi pathways. Essentially, a ssNA chain is designed to complement a specific mRNA sequence; a sequence that would translate for a protein that is therapeutically interesting to interfere with. The most successful chemistry to date is that of the dimethylamino-phosphorodiamidate MNA (PMO, @fig-sugarmods E.) chemistry and the thiophosphoramidate variant (TMO) has been applied to resolve a genetic defect in a particular variant of the Duchenne's musculodystrophy disease #mcite(("Roshmi2019viltolarsen", "Langner2020TMO"), biblio). 
...
It is common practice in biotechnological research and production to invest resources in the management of bacterial cultures and extraction of the target molecule(s), i.e. upstream and downstream processing respectively. A good product should be void of impurities and external invasion of pathogens can disrupt and destroy manufactured batches of proteins, which poses a real problem in the development of vaccins for example.
...
In biotechnological industries, purifying products and maintaining purity of the colonies is one of the most expensive aspects and this system would tremendously cut its costs #mcite(("Groaz2023hna"), biblio).
