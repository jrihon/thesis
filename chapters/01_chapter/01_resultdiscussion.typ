...
//
//
Additionally, the _geometry_ module is supplemented with three functions to calculate molecular geometries (bond length, bond angle and dihedral). These works by passing in coordinates from parsed molecule files.
...
@fig-supplementary-deltaEnergy compares all SPE calculations to the GSQ ($ Delta Delta E$), @fig-supplementary-deltaRMSD compares all optimised geometries from the various LoTs with one another, for a holistic depiction of the results of this experiment.
//#figure(
//  image("./figures/delta_PDF.svg"),
//  caption : [
//            *A.* Comparison of the difference in relative energy ($Delta E$) between the GSQ (mp2q) CS experiments and the other geometry optimisation sampling who've been subjected to an SPE at the  mp2q level. Ranges from [-0.5 $arrow.r$ 0.5] $frac("kcal","mol")$)
...
Around a latitude of 55$degree$, we encounter the Envelope (#super("X")E, E#sub("Y")) and Twist (#super("X")T#sub("Y")) (example in @fig-InversionConf B.) puckering modes, while at 135$degree$ we see their inverse puckering modes (#super("Y")E, E#sub("X") ; #super("Y")T#sub("X")). Twists are often referred to as halfchairs and define a conformation where only two consecutive atoms both exist out-of-plane, one on either side of the plane. @fig-sixring gives an overview of the six-membered ring puckering modes.
