...
The accelerated methodology (Mattelaer _et al._) employs two levels of theory to dramatically speed up the duration of the experiment, by applying a cheap one for the GO stage (\hfthreec) and an expensive one for the SPE stage (mp2q) to accurately sample the behaviour of the molecules subjected to this study.#mcite(("Mattelaer2021efficient"), biblio) All this comes together to generate a PES that represents the behaviour of the studied molecule.
