...
Generating a landscape supplies the user with constraints, which are then imposed on the respective dihedrals of the molecule; $(phi, psi)$ for peptides, $(nu_1, nu_3)$ for five-membered rings and $(alpha_1, alpha_2, alpha_3)$ for six-membered rings. These constraints, together with the axes of the grids, are produced by the _pucke.rs_ toolkit. The constraints are applied during GO procedures, which generate all the possible conformations that molecule's system can adopt, given that the query amply covers the landscape.
The accelerated methodology #mcite(("Mattelaer2021efficient"), biblio) employs two levels of theory to dramatically speed up the duration of the experiment, by applying a cheap one for the GO stage (hf3c) and an expensive one for the SPE stage (mp2q) to accurately sample the behaviour of the molecules subjected to this study #mcite(("Mattelaer2021efficient"), biblio). All this comes together to generate a PES that represents the behaviour of the studied molecule.
...
//
==== The pucke.py module
In order to make the methodology more user-friendly, we provide both a CLI tool and a scripting library (_pucke.py_, a Python-wrapped Rust library) to allow users to implement either option into their own workflows.
The Rust language was a deliberate choice to ensure the robustness of the toolkit.
In the Python module, the CLI-tool has been implemented as the _conformation sampling_ module. Additionally, the _pucke.py_ module contains the _formalism_ module that allows the user to calculate various puckering formalisms for five- (AS, CP5) and six-membered (SP, CP6) rings. Furthermore, the user can pass specific Cremer-Pople coordinates to the CP5($r, phi_2$) or CP6($r, phi_2, theta$) class and invert these parameters to produce the 3D structure they have defined, as an xyz- or pdb-formatted file. 
...
//To reiterate, the mp2q is represented as the GSQ and its result is located in the bottom right corner of @fig-PESALL.
Obtained PESs are compared with @tbl-wallclock) and the Consumables (Suppl. Fig. SIConsumables).
...
//
//
The selected GSQ, mp2q, requires the heaviest computations of them all, clocking in at roughly 548h or about 22.8 days of calculations. The geometry optimisation capped out at about 48 GiB of RAM, while at most 40 GiB of _tmp_-files were stored on disk by ORCA when ten conformations were optimised concurrently.
...
The conclusion remains that the (hf3c - mp2q) combination still is a robust competitor in approximating the GSQ, as these calculations are perfectly manageable within a single workday, on hardware with 64 GiB of RAM and 32 cores. The SPE calculations at the pbeq level are a strong candidate for cases where hardware systems are limited as the landscapes evaluated at this level resembles the shape of the mp2q SPE the most. Still, the established methodology #mcite(("Mattelaer2021efficient"), biblio) returns a comparatively amazing result for the frugality of its expenditure. To combat the storing of _tmp_-files, one allocates more RAM per thread. In the interest of the research, a concession had to be made towards the cheaper LoTs , by not exaggerating the resources allocated with respect to their computational requirements.\\
...
To evaluate the quality of the different PESs, a closer look to the differences of the best results is required. An outcome of these CS experiments is to derive parameters for Molecular Dynamics simulations or understand the molecule's behaviour to apply constraints in NMR structural determination experiments. Therefore, the approximation needs to be as close as we can get it to the GSQ.
...
//Overall, the pbeq scores the best on both facets of the analysis.
