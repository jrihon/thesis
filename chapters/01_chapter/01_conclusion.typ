...
== Conclusion
//
Molecular dynamics simulations have proven to be useful to gain insight and predict how changes in the backbone impact on the complementation properties of proteins and (synthetic) nucleic acids. Accurate parametrisation of the force field used in molecular dynamics is a prerequisite to obtain reliable results. Available tools to perform force field parametrisation rely on transitional pathways for conformational changes in fragments of the biomolecules that can be derived from a PES. Here we introduce the _pucke.rs_ CLI and _pucke.py_ library that streamline calculation of PESs for amino acids, five and six-membered ring systems. The _pucke.rs_ toolkit and the conformational sampling methodology synergises with the Ducque model builder #mcite(("Rihon2024Ducque"), biblio) , where evaluated conformers can be curated from the landscape to use as building blocks for XNA model building.
