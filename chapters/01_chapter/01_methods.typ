...
The conformational sampling methodology consists of three parts, starting with the generation of torsion angle constraints to produce initial conformations, which cover the full conformational landscape.
...
Using either tools requires the user to specify the type of molecular system (-\-peptide, -\-fivering or -\-sixring) to be called. Both the peptide and fivering query employ a _linear space_ function, which asks the user to pass an amount of points to be computed for in a preset range.
...
Additional exocyclic constraints were applied to the dA monomer to generate biologically representative five-membered ring conformers for the conformational sampling experiment. $beta$: 180.1$degree$, $gamma$: 60.0$degree$, $epsilon$: 180.1$degree$ and $chi$: 210.59$degree$.

For conformational sampling of a six-membered ring, a set of datapoints is generated on the surface of the CP-sphere that represents the conformational space of six-membered rings. The CP formalism lets us invert from spherical coordinates to atomic coordinates, which are then used by the Strauss-Pickett formalism to convert this information to impromper dihedrals ($alpha_1$, $alpha_2$, $alpha_3$) (@fig-spimpropers). These are used as constraints to generate initial structures in the conformational sampling methodology (@fig-ConfSamplingExplained C.)
...
=== Quantum mechanics
The conformational sampling methodology is subjected to various computational chemistry approaches to assess consumption of resources and quality of their results.
...
This LoT is accompagnied by the 6-311++G (2df,2p) basis #mcite(("Krishnan1980molorbital", "Frisch1984diffusemolorbital"), biblio), with the Resolution of Identity (RI) approximation #mcite(("Neese2003rijk"), biblio). The def2-QZVPP/C auxiliary basis set #mcite(("Hellweg2007auxqzvpp"), biblio) is used for the RI approximation of the MP2 density, together with the def2/JK for approximation on Coulombic and Exchange integrals #mcite(("Weigend2007def2jk"), biblio), hereafter mp2q. The same basis set and approximations are used for the conformational sampling methodology at the _ab initio_ Hartree-Fock (HF) level (hfq). 

The semi-empirical HF-3c LoT is employed for geometry optimisations as it is fast and cheap and before due to its cost efficiency and is employed here to compare against the other methods. This methodology uses the HF level of theory using a basis set of minimal quality (MINIX), with the addition of dampening London dipersion effect (D3 correction), correcting basis set superposition errors using the geometrical counterpoise method and applies a short-range basis incompleteness correction for the overestimation of bondlengths when using small basis sets #mcite(("Kruse2012hf3c", "Sure2013hf3c"), biblio). The double hybrid functional PBE0, with the D4 dampening, uses the same basis as mp2q but with the def2/J auxiliary set #mcite(("Adamo1999pbe0",  "Grimme2010dftcorrection", "Grimme2011dispersion", "Weigend2006def2j"), biblio) (pbeq). Both hf3c and pbeq have shown to output optimised structures of great quality for a fraction of the cost of the pure  wave function theory methods. All geometry optimisations are performed with the _VeryTightOpt_ keyword.
...
The benchmarking consists of comparing different LoTs' resources and assessing their RAM usage, wallclock time and disk space. These are considered the _consumables_ used during computations, which are used to compare against the consumables of the GSQ.
The quality of the respective geometry optimisations are compared by a pairwise structure evaluation, where differences are measured with the Kabsch RMSD algorithm #mcite(("Kabsch1976rmsd"), biblio) (#link("www.github.com/charnley/rmsd")[github.com/charnley/rmsd]).
