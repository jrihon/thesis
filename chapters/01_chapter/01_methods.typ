...
The _pucke.py_ contains this functionality on the _confsampling_ module and works analogously.
...
For the showcase at the end of the chapter (@fig-ShowcasePES A.), the peptide landscape was queried to produce the axes ($phi$, $psi$) at an interval of 10$degree$ to generate 1369 distinct sets of ($phi$, $psi$)-constraints, for the inclusive range of [0. $arrow.r$ 360.]. The L-Alanine (Me-NH-Ala-CO-Me) was used to sample the peptide space (@fig-ConfSamplingExplained A.) and did not require additional constraints. The axes were later transformed to [-180. $arrow.r$ 180] for visual comparison with energetically favourable regions for the peptide backbone, with the popular Ramachandran plot #mcite(("Rosenberg2023ramachandran"), biblio).

For the dA ribose ring, the inclusive range [-60. $arrow.r$ 60.] was queried for an interval of 21, which returns a set of points at an interval of 6$degree$ on the $Z_x$ and $Z_y$ axes, defined by Huang _et al._ #mcite(("Huang2014improve"), biblio).
...
The Gold Standard Quality (GSQ) was decided to be the Møller–Plesset 2#super("nd") order perturbation theory (MP2) #mcite(("Cremer2011mppt"), biblio), as the CI-CCSD(T) LoT does not lend itself well to geometry optimisations and still a remains a demanding LoT, though incredibly accurate.
...
The PES itself is expressed as the relative difference in energy ($Delta E$) of all conformations with respect to the global minimum of the landscape. To evaluate the produced PESs, the differences in relative energy($Delta Delta E$) are compared, for all combinations of LoTs, against the GSQ. This to evaluate which combinations (GO-SPE) can produce qualitative PESs (@fig-PESALL).
