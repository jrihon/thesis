...
The Cremer-Pople (CP) sphere represents the conformational space of six-membered rings. A set of datapoints is generated on the surface of the CP-sphere. The CP formalism lets us invert from spherical coordinates to atomic coordinates, by which the Strauss-Pickett formalism convert this information to impromper dihedrals ($alpha_1$, $alpha_2$, $alpha_3$). These are used as constraints are used to generate initial structures in the CS methodology (@fig-ConfSamplingExplained C.)
