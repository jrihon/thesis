...
In #chapter-style()[Chapter 2], we focus on the methodology of Conformational Sampling. This type of experiment returns us Potential Energy Surfaces with which we can understand the puckering modes a molecule of interest favours to adopt and which one it does not tend to visit. We have previously applied this on modified nucleic acids that are of interest to our lab, through an accelerated methodology that makes it manageable to run on a local workstation. However, understanding and applying the mathematical descriptions used to generate the landscapes can be cumbersome, which is why we introduce to _pucke.rs_ toolkit to simplify the scripting approach to this method. Alongside a benchmarking study to gather insightful data on the differences in methods of computational chemistry, we provide a commandline tool and a Python library that should aid any researcher in applying the methodology for their own molecules without having to trouble themselves with the algebra that goes on behind it.  
