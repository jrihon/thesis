...
Antibodies are typically produced by introducing a protein of interest to the immune system of a living organism. The goal is for the immune system to recognise the protein as a foreign intruder, thereby encouraging the immune system to develop targetted antibodies. The maintenance of such animals brings about many ethical dilemmas and the downstream processing of such Abs costs an extraordinary amount of resources to generate and extract these molecules. Additionally, should these be used for therapeutic indications, the patient themself might reject the administered drug over time by producing their own anti-Abs, rendering these therapies useless over time. A cheap and non-immunogenic alternative to Abs is therefore highly sought after in the world of medical sciences #mcite(("Golay2012Abreview", "Muyldermans2020Nbreview"), biblio).

//==== Systematic evolution of ligands by exponential enrichment (SELEX)

#let content-selex = [In contrast, the conception of new aptamers is realised through the SELEX method (Systematic evolution of ligands by exponential enrichment). This methodology requires no interference of living organisms and can be entirely done _in vitro_. It employs general Polymerase Chain Reactions (PCRs), an extremely common methodology to produce a quantitatively amplified pool of nucleic acid strands. The researchers start with a randomised library of 10#super("8") - 10#super("14") amounts of such strands. These molecules are incubated with the target. The goal is to gather as many good binding aptamers as possible to the target molecule, after which non-functional strands are washed away after incubation. These binders are then dissociated from the target and amplified using the PCR step (@fig-selex). Multiple cycles are required to end up with a pool of strands that are theoretically specific enough to hone in on the desired target while also portraying a high affinity to it. A major upside to aptamers is that, since they are composed of nucleic acid compounds, they rarely elicit immunogenic reactions. The downside however, is that aptamers are prone to efficient degradation in the human bloodstream, as they are composed of native NAs. #mcite(("Mayer2009AptamerReview", "Dunn2017AptamerDiscovery"), biblio)
] 

#let figure-selex = [#figure(
  image("./figures/selex-process.svg", width: 100%),
  caption: [Simplified schematic representation of the inner workings of SELEX . *A.* The target is  incubated with a pool of randomised sequence strands. *B.* Non-functional strands are washed away and the remaining candidate aptamers are *C.* amplified through a PCR reaction. This cycle is continued several times to achieve a library of binders specific to the desired target.],
) <fig-selex>]
...
Since SELEX depends on an NA polymerase for the PCR step, the development of nuclease-resistant XNA aptamers could only be realised if such an XNA processing enzyme could be engineered.
//by feats of engineering in the field of _Directed Evolution_ (DE).
The work "_Synthetic genetic polymers capable of heredity and evolution_" details the _Directed Evolution_ (DE) paradigm to modify a wildtype (WT) TgoT polymerase to process the HNA chemistry. This polymerase was then applied in rounds of SELEX to procure the first HNA aptamer to exist #mcite(("Pinheiro2012geneticpolymer"), biblio).
The following section summarises the relevant work from this scientific publication.
...
In theory, the different plasmids contain a variety of mutations to the polymerase gene to increase the odds of producing an enzyme that carries out successful extensions with an XNA chemistry. A suspension of bacteria, together with XNA nucleotide triphosphates (xNTP) and biotinylated primers, is emulsified in a water-oil emulsion. These tiny oil droplets, which contain the three aforementioned components (bacteria with random plasmid, xNTPs and primer), create the compartiments for the subsequent experiment. 
The bacteria are lysed through heat disruption, exposing its intracellular content in the compartiment. The mutated TgoT polymerase, that have been expressed by the bacteria, can then process the bacterial plasmid by making use of the biotinylated primer that has adhered to a specific sequence of the plasmid. Only xNTPs are present, which pressure the system to only make use of synthetic building blocks. If the enzyme was positively mutated, a complementary strand of XNAs is appended to the primer and the primer-plasmid complex will remain intact for the remainder of the experiment.
