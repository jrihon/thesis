...
These are typically produced by introducing a protein of interest to the immune system of a living organism, oftentimes in rodents and camelids. The goal is for the immune system to recognise the protein as a foreign intruder, thereby encouraging the immune system to develop targetted antibodies. The maintenance of such animals brings about many ethical dilemmas and the downstream processing of such Abs cost an extraordinary amount of resources to generate and extract these molecules. Additionally, should these be used for therapeutic indications, the patient themself might reject the adminstered drug over time by producing their own anti-antibodies, rendering these therapies useless over time. A cheap and non-immunogenic alternative to Abs is therefor highly sought after in the world of medical sciences.

//==== Systematic evolution of ligands by exponential enrichment (SELEX)

#let content-selex = [In contrast, the production of aptamers is realised through the SELEX method (Systematic evolution of ligands by exponential enrichment). This methodology requires no interference of living organisms and can be entirely done _in vitro_ . This employs general Polymerase Chain Reactions (PCRs), an extremely common methodology to produce a quantitatively amplified pool of nucleic acid strands. The researchers start with a randomised library of 10#super("8") - 10#super("14") amounts of such strands. These molecules are incubated with the target. The goal is to gather as many good binders as possible to the target molecule, after which non-functional strands are washed away after incubation. These binders are then dissociated from the target and amplified using the PCR step. Multiple cycles are required to end up with a pool of strands that are theoretically specific enough to hone in on the desired target while also portraying a high affinity to it. A major upside to aptamers is that, since they are composed of nucleic acid compounds, they rarely elicit immunogenic reactions. Flipping the proverbial coin, we do find aptamers to be prone to efficient degradation in the human blo-] 

#let figure-selex = [#figure(
  image("./figures/selex-process.svg", width: 100%),
  caption: [Schematic representation of the inner workings of _Selex_ . *A.* The target is  incubated with a pool of randomised sequence strands. *B.* Non-functional strands are washed away and the remaining candidate aptamers are *C.* amplified through a PCR reaction. This cycle is continued several times to achieve a library of binders specific to the desired target.],
)]
...
#v(-0.4em)
//blo
odstream, as they are composed of DNA and RNA building blocks. 

=== Prologue to the HNA aptamer
//This leads to the following engineering of a HNA polymerase.
Since SELEX depends on such a NA polymerase to function properly, the development of nuclease-resistant XNA aptamers could only be realised by such feats of engineering in the field of _Directed Evolution_ (DE). The work "_Synthetic genetic polymers capable of heredity and evolution_" details on the DE paradigm to modify a wildtype (WT) TgoT polymerase to process the HNA chemistry. This polymerase was then applied in rounds of SELEX to procure the first HNA aptamer to exist #mcite(("Pinheiro2012geneticpolymer"), biblio).
...
The goal is to introduce mutations to the polymerase enzyme and select enzyme candidates that process the desired XNA. 

//A library of plasmids, containing the gene of the TgoT polymerase, are individually expressed in separate bacteria.
A library of plasmids is generated, which contain randomly mutated copies of the gene of the TgoT polymerase. Separate bacteria are transfected with these plasmids and they express their particular copy of the polymerase enzyme.
...
The bacteria are lysed through heat disruption, exposing its intracellular content in the compartiment. The mutated TgoT polymerase can process the bacterial plasmid by making use of the biotinylated primer that has adhered to a specific sequence of the plasmid. Only xNTPs are present, which pressures the system to only make use of synthetic building blocks. If the enzyme was positively mutated, a complementary strand of XNAs is appended to the primer and the primer-plasmid complex will remain intact for the remainder of the experiment.
