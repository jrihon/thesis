#import "../../lib/layout.typ": headingL5, pdb-code, FONT-10PT, FONT-9PT, FONT-11PT, INSET-5PT
...
#show table.cell.where(y: 3): set text(size: FONT-9PT)
...
===== Epitope mapping
An alanine scan, meaning select residues are mutated from amino acid _X_ to an alanine residue, was carried out on the HEL enzyme to deduce which amino acids are crucial to the binding interaction between the nanobody and the aptamer. This study reveals that the nanobody's binding is mainly directed by the Y53, D66 and R84 surface residues (@fig-hel-heatmap B.), while the aptamer is more affected by mutations on the neighbouring residues R61 and R73 (@fig-hel-heatmap C.).
...
===== Charge Derivation
The R.E.D. server #mcite(("Vanquelef2011RED"), biblio) was used to compute point charges of the nucleosides. To make AMBER compatible charges, the molecule needs to be subjected to the Merz-Kollman population analysis scheme (MK) at the HF/6-31G\* level #mcite(("Singh1984MK"), biblio). The #conform("C", "4'", "1'") conformation of the four different nucleosides (hA, hC, hG, hT) were applied to this protocol. The sugar moieties were equivalenced across all four molecules. Identical atoms, like the hydrogens to the amine off the purine ring, are also equivalenced individually. The returned electrostatic potentials (ESPs) were further refined using the Restrained ESP (RESP) fitting protocol #mcite(("Bayly1993RESP"), biblio) to account for buried carbons in methylenes in the sugar moiety. The buried carbon atoms are a results of the population analysis scheme that makes use of the Solvent-Excluded-Surface (SES) volume around the molecule. The grid generated on this surface is used to derive the ESP itself. The hydrogens in methylene moieties therefore _bury_ the carbon, obfuscating its stake during the MK charge derivation; RESP corrects this.
...
At first, a conformational path through which the HNA chemistry could naturally move through was researched. Conformations were optimised using the ORCA computational chemistry package #mcite(("Neese2020ORCA"), biblio), employing the PBE0/ma-def2-svp method #mcite(("Adamo1999PBE0", "Zheng2010madef2"), biblio).  After having defined these pathways (@fig-pathway), a Nudged Elastic Band #mcite(("Asgeirsson2021NebTS"), biblio) protocol was applied to gather intermediary conformations between the established puckering modes. A total of 192 total conformations were fitted, meaning 48 distinct and intermediary conformations per nucleobase, to condense the behaviour of the HNA chemistry into a force field. The fit itself was done using the _Paramfit_ script, provided within AMBERTools #mcite(("Betz2014paramfit"), biblio).
]
...
#figure(
  ...
  caption: [
    ...
    *B.* Second pathway goes #conform("C", "4'", "1'") $arrow.r$ #conform("H", "O'", "5'") $arrow.r$ #conform("S", "O'", "2'") $arrow.r$ #conform("H", "3'", "2'") $arrow.r$ #conform("C", "1'", "4'"). 
    Conformations generated using the _pucke.py_ library. Blue ball on the _C2'_ carbon denotes the nitrogen that is part of the glycosidic bond between the nucleobase and sugar moieties and represents the nucleobase itself.
  ]
) <fig-pathway>
]
...
A methodology that virtually increases the thermostat during a simulation run, in order to overcome energy barriers imposed by the force field, as a result of the nature of the molecule itself. This is characterised by a heating segment up to a preset temperature, a segment that allows the simulated molecule(s) to discover new conformational pathways as the barriers can now be crossed, and a cooling segment to relax the model. This is often done to explore the conformational diversity in molecules.
...
A K#super("+") ion was placed at the center of the Gq structure in the initial model , four Na+ and 26 K+ ions were added to represent experimental conditions and neutralize the molecule. 5646 TIP3P water molecules were added in a truncated octahedron that extends 12 Å from the molecule. After iterative rounds of MD, it was decided that a Mg#super("2+") ion was to be placed between phosphate moieties of hG#sub("20") and hT#sub("12").  Prior to the 1 µs unrestrained MD, a 50 ns restrained MD was performed with restraints on the residues of the Gq, the TAT triplet and the two stabilising ions. For all simulations, a cutoff value of 10 Å was used for long range and electrostatic interactions.
