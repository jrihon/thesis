...
This could then be done for all conformations we want to include in the force field and apply a fitting procedure to it. To highlight what I mean, the supplementary information of the publication, released by Villa's group on the LNA molecule #mcite(("Xu2017lnafit"), biblio), is one of the few papers I have encountered where parameter fitting for torsional angles in nucleic acids has been done explicitly. Fitting torsional parameters for multiple conformations at once, now that's a challenge I would've loved to tackle.
...
One problem with nucleic acid helices is that they are periodic in nature. This continuous periodicity gives way to a relatively constant magnitude and direction of the tangent vector at every knot, meaning there will be little to no difference between subsequent ($kappa, tau$) at every knot in the trace. While the tangent vector might be vary slightly between A-type and B-type structures, their ($kappa, tau$) parameters will always result in equivalent values due to the nature of a continuous helix. 
