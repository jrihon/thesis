...
The ORCA QM package does not contain the MK population analysis scheme. To keep the workflow free, local and easy to use, we implemented the population analysis scheme for ORCA to generate ESP potentials that are ready to use for the RESP fitting.
...
With respect to wallclock building-time, NAB averaged 0.272 s and 0.245 s for DNA and RNA respectively. Ducque, averaging 0.328 s and 0.234 s for DNA and RNA respectively, takes slightly longer for DNA as it considers multiple conformers during the fitting (benchmarked with Python3.12).
...
The PESs of the four morpholino nucleosides successfully provided valuable information on transitional pathways. By using the improper dihedrals ($alpha_1$, $alpha_2$, $alpha_3$) as restraints on the pyranose ring, we gained information of possible transitions of conformations as well as puckers we should be expecting. 
...
As this obvious abnormality is attributable to an odd occurrence of the simulation and not the FF itself, data of mA30 and m31 are excluded the table in @tbl-suppl-RNAMNA_backbone. This behaviour was not observed in the preliminary RNA::MNA simulations.
