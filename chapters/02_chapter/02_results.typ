...
The ORCA QM package does not contain the MK population analysis scheme. To keep the workflow free, local and easy to use, we implemented the population analysis scheme for ORCA to generate ESP that are ready to use for the RESP fitting.
...
Our methodology shows an even distribution of the surface, compared to GAMESS, and decided to proceed with this approach as it produced a near-identical range of ESP values to that of GAMESS'.
A benchmarking against RESP charge derivation by the R.E.D. server #mcite(("Vanquelef2011redsa"), biblio) was carried out on HNA nucleosides. We see near-identical atomic charges being fitted to the atoms in the respective nucleosides. 
Data points from either methodology were compared by pairwise atom alignment (linear regression, SciPy) per nucleoside. This results in an $r^2$ of 0.985 for hA, 0.985 for hC, 0.987 for hG and 0.979 for hT. This result implicitly validates the grid generation aspect of the ORCA implementation.
//
//
//
//
...
With respect to wallclock building-time, NAB averaged 0.272 s and 0.245 s for DNA and RNA respectively. Ducque, averaging 0.328 s and 0.234 s for DNA and RNA respectively, taking slightly longer for DNA as it considers multiple conformers during the fitting (benchmarked with Python3.12).
...
To assess the quality of Ducque's models for MD simulations, a series of duplexes were studied, starting with (deoxy)-ribose based nucleic acids, which employed the standard AMBER DNA.OL15 and RNA.OL3 force fields. The simulated structures for the DNA and RNA homoduplex experiments, generated by both NAB or Ducque, show equivalent RMSD trajectories, as expected given the similarity of the starting structures. An MD simulation on a B-type DNA::RNA and A-type RNA::DNA duplex of identical sequence both built by Ducque, converged into the same A-like structure, as experimentally determined in Davis _et al._ #mcite(("Davis2015rnadna"), biblio).
...
//To emphasise, the HNA parameters were derived entirely from QM \cite{mattelaer2021PHD, Schofield2023HnaAptamer}. \textit{Nota bene}, so have the DNA and RNA torsion angle parameters, who've later on been tweaked based on additional QM studies and a collaborative effort with NMR \cite{cornell1995secondGenforce field,zgarbova2011RNAOL3,zgarbova2015DNAOL15, Perez2007AlphaGamma}.
//
//
//
//
...
Following the procedure described by Mattelaer _et al._ #mcite(("Mattelaer2021efficient"), biblio), PESs were generated for the morpholino nucleosides with adenine, guanine, thymine and cytosine nucleobases. We observed intramolecular hydrogen bonds in the initially unrestrained geometry optimised calculations, inflating equatorial conformations towards higher energy potentials. It concerns hydrogen bonds on the terminal ends of the nucleoside. Those observed hydrogen bonds, which would not exist in a polymeric molecule, introduce a forced steric strain on the nucleoside that //would not yield representative force field parameters that are applicable for simulations on oligonucleotides. For the capped MNA variant (@fig-MethylMorpholinoPES, @tbl-ErelConformers), we see a global minimum on the north pole of the CP-sphere, a #super("4'")C#sub("1'") configuration for all four nucleosides. Local minima are observed mainly on the upper hemisphere, separated by large peaks corresponding to envelope configurations. Equatorial puckering modes tend to be more energetically favourable, with boats and skews alike. A final local minimum is observed on the southern hemisphere, in the East.
]
#grid(fig-confs-morph, content-confs-morph, columns: (7.4fr, 5fr), column-gutter: 1em)
#v(-0.5em)
...
The PESs of the four morpholino nucleosides provided valuable information on transitional pathways. By using the improper dihedrals ($alpha_1$, $alpha_2$, $alpha_3$) as restraints on the pyranose ring, we gained information of possible transitions of conformations as well as puckers we should be expecting. 
Conformations highlighted in @tbl-ErelConformers are conformations that were initially considered for parametrisation, specifically for fitting of the sugar torsion angles (see @fig-sixring B. for a reference to their respective coordinates on the CP-sphere).
...
//
//
Coordination of K#super("+") by N3_'_ in the morpholino ring of mA30 forces it into metastable configurations (@fig-ion2bbjustfig), as can also be seen on the scatterplots, around the equator (@fig-fullRNAMNA C).
As this obvious abnormality is attributable to an odd occurrence of the simulation and not the force field itself, data of mA30 and m31 are excluded the table in @tbl-suppl-RNAMNA_backbone. This behaviour was not observed in the preliminary RNA::MNA simulations.
...
//
//
Except for the $chi$ dihedral angle with a deviation of 2.39-2.93$degree$, no significant differences in average backbone angles in the RNA backbone could be observed in the RNA::MNA duplex versus the model obtained for dsRNA (@tbl-suppl-RNAMNA_backbone).
