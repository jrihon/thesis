...
%Two parameters define a force field : a set of charges and the geometrical properties of the molecules.
...
The MK population analysis scheme has been implemented, by the authors, for ORCA and detailed here below. Geometry optimisation of the molecules was done at the PBE0 level, which was followed by an SPE at the HF/6-31G\* level/basisset, with ORCA, to generate the correct molecular density orbitals for RESP #mcite(("Cieplak1995respnuc"), biblio). The gridpoints are generated through the MSMS software #mcite(("Sanner1996surface"), biblio) at a range of factors times (1.4, 1.6, 1.8 and 2.0) the van der Waals (vdW) radius, defined by the Connolly surface algorithm #mcite(("Connolly1983analytical"), biblio). The probe radius was set to 1.4 Å, the density of the points on the surface was set to 3 /Å#super("2") (Figure MKPopAnalScheme).
