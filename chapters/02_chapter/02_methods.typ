...
The generated spherical coordinates ($r$, $theta$, $phi$) are converted to local elevation ($z_j$) from a mean plane per respective atom #mcite(("Cremer1975general"), biblio), by assuming bondlengths and angles, which positions the atoms in the ring in cartesian space. From the atomic coordinates, the improper dihedrals ($alpha_1$, $alpha_2$, $alpha_3$) can be calculated for #mcite(("Strauss1970conformational"), biblio). Graphical representations of these formalisms have been added (@fig-sixring, @fig-spimpropers).
These improper dihedrals are used as restraints for the ORCA software (v. 5.0.2) in geometry optimisation protocols #mcite(("Neese2020orca"), biblio). Several backbone torsion angles were restrained as follows : $beta$ (180.1$degree$), $gamma$ (60.1$degree$), $epsilon$ (180.1$degree$), $chi$ (193.9$degree$) #mcite(("Mattelaer2021efficient"), biblio).
First, a lightweight semiempirical HF-3c (Hartree Fock) geometry optimisation is performed to relax the respective conformations after imposing the restraints. Afterwards, a single point energy (SPE) evaluation, through Møller-Plesset 2nd order perturbation theory (MP2) is performed to evaluate the potential of the respective conformers. The specific accompagnying basis set for MP2 is 6-311++G(2df,2p) with  the def2-qzvpp/C basis as an auxiliary basis set for the RI approximation and the def2/JK auxiliary basis set for the RI approximation of Coulomb and exchange integrals in the HF step #mcite(("Mattelaer2021efficient"), biblio). A total of 630 conformers were sampled for each morpholino nucleoside.
...
The respective nucleobases were flattened using the HF-3c level geometry optimisation, with the atoms in and on the sugar restrained in cartesian space and the $chi$ dihedral restrained at its current position. The improper dihedral for purines (C1_'_, C8, N9, C4) and pyrimidines (C1_'_, C6, N1, C2) is also restrained to -179.5° to ensure a plane as flat as the standard nucleobases provided with NAB to satisfy the planarity assumptions in the rotations.
...
The AMBER force field #mcite(("Case2005amber", "Salomon2013amberpackage"), biblio) is compatible with restrained electrostatic potential (RESP) charges #mcite(("Bayly1993wellbehaved"), biblio). This is a subset of molecular electrostatic potential (MEP) sampled with the Merz-Kollman (MK) population analysis scheme #mcite(("Singh1984approach", "Besler1990semiempirical"), biblio), followed by a two-stage restrained procedure to derive atomic charges (RESP fitting).
The nucleosides were superposed on the X-Z cartesian plane by the (C1_'_, C5_'_, N3_'_) atoms in the nucleosides through an in-house scripted reorientation, before charge fitting #mcite(("Dupradeau2010redtools"), biblio).
...
MSMS employs a further triangulation of the vertices to distribute them more equally. ORCA's _orca\_vpot_ program is used to map the orbitals and their densities to the respective gridpoints, in atomic units (a.u.). Afterwards, the ESP-loaded grid is provided to the RESP script for a two-stage restrained procedure, deriving point charges for the atoms. Similar atoms are equivalenced and internal fragments are restrained to a set charge for the respective nucleosides, which ultimately returns the needed RESP charges for the force field #mcite(("Bayly1993wellbehaved", "Cieplak1995respnuc", "Aduri2007paramnuc"), biblio).
//
...
A customised CF atom type was created, which was made equivalent to the standard CT atom type, common in AMBER force fields. This was done so as not to override any parameters when loading in the MNA force field together with the standard force fields.
//
//
//
//
